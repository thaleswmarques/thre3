# 🚗 Inventário — R3NT A CAR

> **Última atualização:** 26/06/2026
> Documento de controle da frota, contas a receber, financiamentos e pendências.

---

## 📊 Resumo executivo

| Indicador | Valor |
|---|---|
| Veículos na frota | 6 (1 saindo por perda total) |
| Alugados / gerando receita | 3 (Mônaco, Lyon, Munich) |
| Parados | 1 (Paris — oficina) |
| Saindo / vendidos | 2 (Stuttgart — perda total; Detroit — vendido p/ Vicente) |
| **Custo fixo mensal hoje** | **~R$ 5.015** |
| **Custo fixo após ajustes** | **~R$ 3.447** (economia ~R$ 1.568/mês) |
| **A receber (quantificável)** | **~R$ 60.900** (sendo R$ 57.000 parcelados do Cruze) |

---

## 🚙 Frota — status dos veículos

| # | Nome | Modelo | Status | Locatário | Financiamento |
|---|---|---|---|---|---|
| 1 | Stuttgart | Gol preto | 🔴 Perda total (seguradora) | — | Carta de crédito (João) |
| 2 | Mônaco | C4 Cactus branco | 🟢 Alugado | Marcos Rosa | Carta de crédito (João) |
| 3 | Paris | C4 Lounge cinza | 🟡 Oficina | — (meta: app) | Panamericano (mãe) |
| 4 | Lyon | C4 grafite | 🟢 Alugado | Lucas | Carta de crédito (João) |
| 5 | Munich | Fox branco | 🟢 Alugado | Marcos do Lana | Quitado (mãe) |
| 6 | Detroit | Cruze | 🔵 Vendido | Vicente (comprador) | Quitado (seu nome) |

### 1. Stuttgart — Gol preto 🔴
- **Status:** perda total na seguradora.
- **Situação financeira:** fica no "elas por elas" — perde só o que já pagou.
- **Ganho:** deixa de pagar **R$ 750/mês** de financiamento.
- ⚠️ **Confirmar com João:** quando começa a redução da parcela (este mês ou o próximo).
- 📅 **Procon:** **29/06, às 14h30** (confirmado) — 📌 cai numa **segunda-feira** (a etiqueta "sexta" do relatório original estava errada). Agendado via Sebrae (dúvidas de CNPJ). Burocrático porque o carro não está no seu nome.
- **Financiamento:** carta de crédito no nome do João.

### 2. Mônaco — C4 Cactus branco 🟢
- **Status:** alugado com Marcos Rosa a **R$ 900/semana** (~R$ 3.900/mês).
- **Condição:** batida na porta do lado do motorista, dois retrovisores remendados.
- 💰 **A receber:** R$ 1.500 de calção do Marcos Rosa.
- 🔧 **Manutenção:** parte é responsabilidade do Marcos Rosa (porta + retrovisores).
- **Ação:** cobrar o dinheiro **+** negociar a responsabilidade da manutenção.
- **Financiamento:** nome do João, carta de crédito (quase R$ 50 mil de entrada com carro).

### 3. Paris — C4 Lounge cinza 🟡
- **Status:** na oficina, esperando peça do Mercado Livre (a anterior foi devolvida por defeito).
- **Objetivo:** liberar para trabalhar como motorista de app.
- 💰 **A receber:** R$ 1.000 de vidros (seguradora) + R$ 500 do Vicente.
- 🔧 **Manutenção:** bobina, carcaça, leque da bandeja (~R$ 1.500 em peças).
- **Financiamento:** Banco Panamericano, nome da mãe, parcela de **R$ 2.618**.
- ⚖️ **Revisional na justiça:** deve cair para **~R$ 1.800**, em 48 prestações.
- 📄 **IPVA 2025:** última parcela vencida dia 29 do mês passado (CIAR).

### 4. Lyon — C4 grafite 🟢
- **Status:** alugado com Lucas.
- 💰 **A receber:** R$ 900 (vencimento esta semana).
- **Situação:** Lucas provavelmente vai comprar carro próprio e devolver.
- **Financiamento:** carta de crédito João (~R$ 70 mil de saldo devedor).
- 📄 **IPVA 2026:** em aberto.

### 5. Munich — Fox branco 🟢
- **Status:** alugado com Marcos do Lana. **Pagou tudo ontem, regular.**
- **Financiamento:** quitado, nome da mãe.
- 📄 **IPVA 2026:** em aberto.

### 6. Detroit — Cruze 🔵 (vendido)
- **Status:** vendido para Vicente em **3× R$ 10.000 + 19× R$ 3.000 + telefone de R$ 3.000** (já entregue).
- **Já pago pelo Vicente:** as 3 parcelas de R$ 10.000 (com atrasos).
- 💰 **A receber:** 19 parcelas de R$ 3.000 = **R$ 57.000**.
- **Seguro:** você pagando R$ 340/mês desde janeiro (passa pro Vicente quando ele assumir).
- 📄 **IPVA 2025:** parcelamento CIAR, vencimentos dia 29 (vencido) e dia 29 deste mês. Faltam 4–5 parcelas.
- **Financiamento:** estava quitado, no seu nome.

---

## 🗂️ Legenda da frota e veículos pessoais

**Frota de locação (6 carros):** Stuttgart (Gol preto) · Mônaco (C4 Cactus branco) · Paris (C4 Lounge cinza — carro principal de Uber do Thales) · Lyon (C4 grafite) · Munich (Fox branco) · Detroit (Cruze, vendido p/ Vicente).

**Veículos pessoais (fora da locação):**
- **Osaka** — Honda CR-V da mãe (carro reserva; usado quando o Paris está parado).
- **Margot** — moto MT-03 (transporte do Thales).

## 📆 Recebimentos fixos semanais (R3nt Admin)

| Quando | Valor | Provável origem |
|---|---|---|
| Toda segunda | R$ 900 | aluguel semanal (Mônaco ou Lyon) |
| Toda quarta | R$ 600 | Munich (Marcos do Lana) |
| Toda sexta | R$ 900 | aluguel semanal (Lyon ou Mônaco) |
| Todo dia 25 | R$ 3.000 | Vicente (parcela do Cruze) |

> A confirmar qual locatário paga em cada dia. **Vicente:** a parcela do Cruze (R$ 3.000) vence dia 25; em **25/06 ele pagou R$ 1.500** — falta R$ 1.500 deste mês *(a confirmar se era pra essa parcela)*.

## 🤖 Assistente de agenda — "Friday"
O assistente pessoal de agenda no celular se chama **Friday** (conversa fixa **"Agenda Diária"**). Ele toca a rotina/dia; **o Code mantém o planejamento**. Ciclo: Friday gera o **relatório diário** → Thales traz pro Code → Code atualiza o planejamento. Contexto pessoal salvo no Drive (pasta *Contexto Claude*).

---

## 💰 Contas a receber

| Devedor | Referente | Valor | Prazo | Status |
|---|---|---|---|---|
| Vicente | Cruze (19× R$ 3.000) | **R$ 57.000** | Parcelado | A receber |
| Vicente | Paris (acerto) | R$ 500 | — | A receber |
| Vicente | Prejuízo C4 Lounge | *a definir* | — | 🔄 Em negociação |
| Lucas | Lyon (aluguel) | R$ 900 | Esta semana | A receber |
| Marcos Rosa | Mônaco (calção) | R$ 1.500 | — | A receber |
| Marcos Rosa | Manutenção (parte) | *a definir* | — | 🔄 Em negociação |
| Seguradora | Paris (vidros) | R$ 1.000 | — | A receber |
| **Total quantificável** | | **~R$ 60.900** | | |

> ⚠️ **Passivo (caução a devolver):** você segura **R$ 3.500** de caução do Vicente (C4 Lounge), a devolver no fim da locação. Ver compensação na seção do cálculo abaixo.

---

## 🏦 Financiamentos — resumo

| Veículo | Banco / Origem | Titular | Parcela | Observação |
|---|---|---|---|---|
| Paris (C4 Lounge) | Panamericano | Mãe | R$ 2.618 → ~R$ 1.800 | Em revisional (48 prestações) |
| Lyon (C4 grafite) | Carta de crédito | João | parte do bloco João | ~R$ 70 mil saldo devedor |
| Mônaco (C4 Cactus) | Carta de crédito | João | parte do bloco João | ~R$ 50 mil de entrada |
| Stuttgart (Gol) | Carta de crédito | João | R$ 750 | Sai com a perda total |
| Munich (Fox) | Quitado | Mãe | R$ 0 | — |
| Detroit (Cruze) | Quitado | Você | R$ 0 | Vendido p/ Vicente |

**Pagamento total ao João:** R$ 1.700/mês → cai para **~R$ 950** quando a Stuttgart sair.

---

## 📄 IPVA e documentação

| Veículo | IPVA 2026 | IPVA 2025 |
|---|---|---|
| Stuttgart | Em aberto | — |
| Mônaco | Em aberto | — |
| Lyon | Em aberto | — |
| Munich | Em aberto | — |
| Detroit | Em aberto | Parcelamento CIAR — 2 parcelas (29 vencido + 29 deste mês), faltam 4–5 |
| Paris | Em aberto | Parcelamento CIAR — última parcela vencida dia 29 do mês passado |

---

## 📈 Análise financeira mensal

### Custos fixos — antes x depois dos ajustes

| Item | Hoje | Após ajustes |
|---|---|---|
| Bloco João (financiamentos) | R$ 1.700 | R$ 950 *(Stuttgart sai)* |
| Panamericano (Paris) | R$ 2.618 | R$ 1.800 *(revisional)* |
| Seguro Cruze (Detroit) | R$ 340 | R$ 340 *(até Vicente assumir)* |
| Seguro C4 (Paris) | R$ 357 | R$ 357 |
| **Total** | **R$ 5.015** | **R$ 3.447** |

➡️ **Economia projetada: ~R$ 1.568/mês** quando a Stuttgart sair e a revisional do Paris cair.

### Receita de aluguel (conhecida)

| Veículo | Locatário | Valor | Mensal aprox. |
|---|---|---|---|
| Mônaco | Marcos Rosa | R$ 900/semana | ~R$ 3.600–3.900 |
| Lyon | Lucas | R$ 900/semana¹ | ~R$ 3.600–3.900 |
| Munich | Marcos do Lana | *não informado* | — |
| Paris | — (parado) | potencial R$ 4.000/mês | 0 (em manutenção) |

¹ Valor do Lyon assumido igual ao do Mônaco (o relatório só cita o débito de R$ 900 da semana). **Confirmar.**

> 💡 **Liberar o Paris da oficina** recoloca até **~R$ 4.000/mês** (ou a renda de app) e é a maior alavanca de caixa disponível hoje.

---

## 🧮 Cálculo do Vicente — prejuízo do C4 Lounge

> **Pergunta:** quanto o Vicente deve absorver do prejuízo acumulado **sem estragar a relação**?
>
> ⚠️ Cálculo feito a partir do **resumo fornecido** (não dos contratos). Com os contratos em mãos, dá pra fechar os valores exatos.

### Histórico
Vicente alugava o C4 Lounge há anos. No fim de 2025 o C4 parou para manutenção; você emprestou o Cruze a preço reduzido e depois vendeu o Cruze para ele.

### O que o Vicente já pagou (relacionado)
| Item | Valor |
|---|---|
| Manutenção do Cruze | R$ 3.500–4.000 |
| Caução do C4 Lounge | R$ 3.500 *(você segura / deve devolver)* |

### O que você gastou no C4 (do bolso)
| Item | Bruto | Líquido |
|---|---|---|
| Parabrisa | R$ 1.500 | R$ 500 *(R$ 1.000 recuperável da seguradora)* |
| Peças (bobina, carcaça, leque) | R$ 1.500 | R$ 1.500 |
| **Subtotal reparos** | **R$ 3.000** | **~R$ 2.000** |

### Seguros que você carregou
| Item | Cálculo | Valor |
|---|---|---|
| Seguro Cruze | R$ 340 × 6 meses (jan–jun) | R$ 2.040 |
| Seguro C4 (período parado) | R$ 357 × período | estimativa |

### Receita não realizada (custo de oportunidade)
- C4 Lounge aluga normalmente por **R$ 4.000/mês**.
- Ficou parado **~6 meses** (~3 com Vicente a preço reduzido, ~2 na sua mão, ~2 na oficina).
- **Receita não realizada: ~R$ 24.000.**

### Posição líquida entre vocês
- Você **segura R$ 3.500** de caução do Vicente (deve devolver).
- Prejuízo "duro" (do bolso) que poderia ser dividido: **reparos líquidos ~R$ 2.000 + seguro Cruze carregado ~R$ 2.040 ≈ R$ 4.000**.
- O **custo de oportunidade (~R$ 24.000)** normalmente **não se cobra integralmente** de um cliente de relação longa — entra como argumento de negociação, não como fatura.

### 🎯 Cenários — quanto o Vicente absorve do prejuízo do C4

| Cenário | Lógica | Vicente absorve | Acerto com a caução (R$ 3.500) |
|---|---|---|---|
| **A — Conservador** *(preserva ao máximo a relação)* | Metade dos reparos líquidos | ~R$ 1.000 | Você devolve **~R$ 2.500** |
| **B — Equilibrado** ⭐ *(recomendado)* | Reparos líquidos cheios + 1 mês de uso/seguro | ~R$ 2.500–3.000 | Caução **praticamente zera** (devolve ~R$ 500–1.000 ou ficam quites) |
| **C — Recupera custo duro** | Reparos + seguro Cruze do período + simbólico de oportunidade | ~R$ 5.000–6.000 | Abate a caução; Vicente ainda fica devendo **~R$ 1.500–2.500** (soma às parcelas do Cruze) |

> **Recomendação:** **Cenário B**. Você recupera o que saiu do bolso (reparos), a caução serve de compensação e a conta "fecha redonda" — sem cobrar os ~R$ 24 mil de oportunidade, que azedariam uma relação de anos. Use o custo de oportunidade só como argumento ("segurei muita coisa nesse período") para justificar não devolver a caução cheia.
>
> 📌 **Para fechar o número exato, preciso:** valor real da manutenção do Cruze (R$ 3.500 ou 4.000?), datas exatas de início/fim de cada fase (Vicente / sua mão / oficina) e quem pagou o seguro do C4 durante a parada.

---

## 🔧 Situação do Marcos Rosa

- Aluga o **Mônaco** (C4 Cactus) a **R$ 900/semana**.
- 💰 **Débito:** R$ 1.500 de calção.
- 🔧 **Manutenção pendente:** porta do lado do motorista + retrovisores (parte é responsabilidade dele).
- **Ação:** cobrar o dinheiro **+** negociar o pagamento da manutenção.

---

## ✅ Prioridades imediatas

1. [ ] **Cobrar Vicente e Marcos Rosa** — liberar capital preso.
2. [ ] **Liberar o Paris da oficina** — gerar receita com app (maior alavanca de caixa).
3. [ ] **Regularizar o Detroit** — últimas parcelas do IPVA (CIAR).
4. [ ] **Confirmar com o João** a redução da parcela da Stuttgart (este mês ou o próximo).
5. [ ] **Fechar o cálculo do Vicente** (prejuízo do C4) — decidir cenário e fechar a caução.
6. [ ] **Procon — sexta 29/06, 14h30** (Stuttgart).
7. [ ] **Cobrar Lucas** — R$ 900 do Lyon (vence esta semana).

---

## 📌 Premissas e pendências (o que falta para fechar os números)

- **Valor do aluguel do Lyon e do Munich** não informados (Lyon assumido em R$ 900/sem).
- **Manutenção do Cruze:** faixa R$ 3.500–4.000 (usar valor exato do contrato/recibo).
- **Seguro do C4 no período parado:** confirmar quem pagou e quantos meses.
- **Datas exatas** das fases do C4 (Vicente / sua mão / oficina) para travar o custo de oportunidade.
- **Manutenção do Mônaco:** definir o % de responsabilidade do Marcos Rosa.
